...
| `power_horizon` explains the elite gap vs silverbot | **Refuted.** The ordinary-fight deficit (−3.42 ± 0.64) is 61% elites and their worst cases are all long high-HP fights — but absolute horizon at hp150/hp300/hp150+2× per-turn gives −0.18 / +0.05 / +0.13 on 497 pool fights. The elite gap stays unexplained, and it is the only *competitive* combat gap left |
| Exhaustive one-turn leaf evaluation beats a greedy playout | **Refuted, and it explains four earlier refutations.** turnsearch wins 0-3/24 vs the rollout's 11/24, and slower. The rollout's virtue is not play quality (0/24 alone) but that it reaches TERMINAL states; static evaluation of a 1-turn horizon cannot compete. Bad play to a real terminal beats good play to a fake evaluation |
| Distilling the clairvoyant solver would teach the student to cheat | **Largely refuted.** 80% of its decisions are identical under permuted hidden draw order (218 decisions, 4 permutations, deterministic chooser), and invariance holds through the turns where deaths cluster. Its edge is exhaustive evaluation, not peeking. An MCTS-based version of this measurement said 14% — but disagreed with *itself* 71% of the time |
| A rollout-free solver finds Heart lines MCTS misses | **CONFIRMED on his real seeds** (the earlier refutation used seed-1 draws whose winnability was never established). Beam 6/8 wins paying 43.9 HP vs MCTS 3/8 paying 78.2 — it beats the human's own HP cost. Winning lines exist and are findable; the teacher exists |
| A better teacher can be built for the Heart | **Refuted.** Clairvoyant @12000, win-bonus ×5, and a survival-first objective all give 11-12 deaths of 20 (control 12). And **silverbot dies 76% there too, paying more HP than us — we already beat it at the Heart by 9 HP** (−30.0 vs −39.2 on identical fights). No teacher exists to distill |
| Boss-power / absolute-horizon levers fix Heart exhaust decks | **Refuted.** Deaths cluster cleanly in exhaust engines, but valuing setup Powers higher does nothing (deaths 141→138-143, HP within noise): the decks die on turns 1-4, before the engine comes online. A setup-survival problem, not a Power-value one |
| Hand-coding the miner's sequencing patterns as scoring terms | **Refuted.** draw-first +0.49/+0.35, burst-debuff +0.49, **both together +0.26** — combined below either alone, doses non-monotone: a positive-mean noise cloud. A flat card-list bonus cannot encode *when*, which is the whole content of sequencing |
| End-state value family (max HP / gold / parasite) | **All three shipped** on behavioral slices: Feed decks bank +0.21 max HP/fight (t = 2.81); Writhing implant rate 48% → 16% (t = −4.31); thief gold pooled +7.2 (t = 2.98). Guards clean. The objective now prices every currency a battle pays |
| Battle-long tree reuse (grafted, 86% hit rate) | Null: −0.05 ± 0.61 at zero cost — fresh sims re-derive the warmth. Closes the docs/13 port audit: of their five pieces, the widening constants were everything |
