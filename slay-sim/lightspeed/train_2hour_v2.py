"""2-hour from-scratch run under the combined reward-v2 (BETA-weighted
incoming damage + widened W_WIN/W_DEATH terminal gap) + observation-v2
(cards_played_this_turn, half_dead, target_is_attacking) changes -- both
confirmed stable via back-to-back 5-minute checks, then a clean 10-minute
run (checkpoint_10min_v2.pt: win 76.3%->83.0%, reward trend -10.86->17.93,
no instability). This is the real comparison point against overnight_v2's
190,500-update baseline (86.8% overall; Collector 41.7%, Reptomancer 25%,
Automaton 66.7%, Awakened One 12.5%, Time Eater 11.1%, Donu & Deca 10.0%) --
2 hours is still short of 8, but the 10-minute run's per-encounter movement
on Automaton/Collector/Reptomancer (all 0%->double digits already) suggests
the new signal is being used; this run is long enough to actually get
meaningful sample counts on the rarest encounters (Time Eater/Awakened
One/Donu & Deca are each ~1/35 of the pool, so they need real update volume
before their win rate means anything).

Same full A0 roster/resources as train_overnight.py.

Run:  PYTHONPATH=. python -m lightspeed.train_2hour_v2
"""
...
FINAL_EVAL_N = 500
OUT_CHECKPOINT = "lightspeed/checkpoint_2hour_v2.pt"
LOG_PATH = "lightspeed/train_2hour_v2_progress.log"
...
def main():
    ...
    _log("=== 2-hour reward-v2 + observation-v2 run (from scratch, A0, full roster) ===")
    _log(f"n_workers={N_WORKERS}, episodes_per_update={EPISODES_PER_UPDATE}")
    ...
    try:
        while time.time() - start < TIME_BUDGET_SECONDS:
            ...
            if any(r != r for r in history):  # NaN check -- r != r is only true for NaN
