"""10-minute from-scratch run under the combined reward-v2 (BETA-weighted
...
already confirmed stable via back-to-back 5-minute checks
(reward_v2_validation_progress.log). This is still not enough training to
expect the specific hard-boss numbers (Collector, Reptomancer, Time Eater,
Awakened One, Donu & Deca) to have caught up to overnight_v2's 190,500-update
baseline -- it's the next data point on the same curve, not a final result.

Same full A0 roster/resources as train_overnight.py, so numbers stay
directionally comparable to the baseline this session identified the weak
spots from.

Run:  PYTHONPATH=. python -m lightspeed.train_10min_v2
"""
...
FINAL_EVAL_N = 300
OUT_CHECKPOINT = "lightspeed/checkpoint_10min_v2.pt"
LOG_PATH = "lightspeed/train_10min_v2_progress.log"
...
def main():
    ...
    _log("=== 10-minute reward-v2 + observation-v2 run (from scratch, A0, full roster) ===")
