"""5-hour from-scratch run: everything from this session's investigation,
combined, against the 8-hour/190,500-update overnight_v2 baseline (86.8%
overall; Collector 41.7%, Reptomancer 25%, Automaton 66.7%, Champ 66.7%,
Awakened One 12.5%, Time Eater 11.1%, Donu & Deca 10.0%).

Everything below was individually verified via real execution (not just
code review) before this run:
  - reward-v2: BETA-weighted incoming damage in the potential + widened
    W_WIN/W_DEATH terminal gap;
  - potential() half-dead fix: a reviving Awakened One contributes -max_hp,
    not 0 (verified: no longer reads as a near-win);
  - observation-v2 (46 state features, up from 13 at session start):
    enemy block, 19 player power stacks, 5 enemy statuses (Poison/Plated
    Armor/Artifact/Metallicize/Mode Shift), the REAL Time Warp counter
    (verified tracking 0.25->0.50->0.75->0.92->reset across turns, fixing
    an earlier version that fed the wrong per-turn counter), draw+discard
    pile composition (4 features), unblocked-incoming-as-fraction-of-HP;
  - action features (12, up from 8): target_is_attacking plus
    target_self_buffs/buffs_ally/debuffs_player via classify_monster_move
    (verified: correctly flags Donu's Buff as buffs_ally=True, Time
    Eater's Ripple as debuffs_player=True, and leaves the real bc
    untouched);
  - monster identity embedding (47 monsters, verified end-to-end including
    the parallel worker path);
  - saturating normalization (x/(x+c)) on every unbounded magnitude
    (block/damage/strength/stacks) instead of the old linear /CONST, which
    under-resolved Barricade/Demon-Form-scale builds;
  - failure-weighted curriculum sampling (EMA-tracked per-encounter win
    rate biases the next chunk's encounter mix toward what's losing);
  - weighted_ironclad_deck: real Ironclad pick-rate data (MaT1g3R/Slay-the-
    Spire-data, ~5500 reward decisions, 50 games, 58% win rate) instead of
    uniform card sampling, with smoothing so 0-pick-rate cards still get
    exposure, and a precomputed cum_weights table so it's not even slower
    than uniform (measured: 0.91x, was 3.49x before that optimization).

The network is substantially larger than the architecture overnight_v2
trained (46 features + monster embeddings vs 13 features, no embeddings),
so 5 hours is not guaranteed to reach parity with 8 hours on the old
architecture -- this is the real data point, not a foregone conclusion.
Final breakdown uses the same weighted_ironclad_deck as training throughout
(that's part of what's being measured) but resets encounter_weights to None
so the ENCOUNTER mix is uniform for the final comparable per-encounter
numbers, same as every prior baseline's final eval.

Run:  PYTHONPATH=. python -m lightspeed.train_5hour_v4
"""
...
CURRICULUM_K = 4.0
EMA_ALPHA = 0.3
OUT_CHECKPOINT = "lightspeed/checkpoint_5hour_v4.pt"
LOG_PATH = "lightspeed/train_5hour_v4_progress.log"
...
def main():
    ...
    _log("=== 5-hour v4 run: reward-v2 + obs-v2 + monster-embed + saturating-norm "
         "+ curriculum + weighted-deck (from scratch, A0) ===")
