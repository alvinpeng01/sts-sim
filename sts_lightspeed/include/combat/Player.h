...
namespace sts {
    ...
    struct Player {
        ...
        int8_t orbSlots = 0; // max channelable orbs (capacity), NOT how many are currently channeled

        static constexpr int MAX_ORB_SLOTS = 10; // real-game practical ceiling; array is fixed-size for a plain-data Player
        std::array<Orb, MAX_ORB_SLOTS> orbs {}; // left-packed (orbs[0] is leftmost/oldest), no gaps
        std::array<std::int16_t, MAX_ORB_SLOTS> orbDarkAmount {}; // meaningful only where orbs[i] == Orb::DARK: the damage this orb will deal next trigger
        int8_t orbCount = 0; // number of currently channeled (non-empty) orbs, <= orbSlots
        int16_t lightningChanneledThisCombat = 0;
        int16_t frostChanneledThisCombat = 0;
        ...
        // Persists across turns: several Watcher cards inspect the card
        // immediately preceding the current card play.
        CardType lastCardPlayedType = CardType::INVALID;
        int16_t mantraGainedThisCombat = 0;
        ...
        // Nightmare: "choose a card, next turn add N copies of it to hand" --
        // the real game implements this with an internal power object that
        // carries an arbitrary CardId, which doesn't fit this project's
        // int-amount-only PS status system, so it gets its own small pair of
        // fields instead (same convention as bomb1/2/3 above for The Bomb).
        CardId nightmareCardId = CardId::INVALID;
        std::int8_t nightmareCopies = 0;
        ...
        void evokeOrb(BattleContext &bc, int slotIdx, int times=1); // triggers the orb's (bigger) evoke effect `times` times (Dualcast evokes twice), then removes it once, shifting the rest left
        void triggerOrbPassive(BattleContext &bc, int slotIdx); // a single orb's end-of-turn tick (Darkness triggers this once, immediately, outside the normal end-of-turn pass)
        void triggerOrbPassives(BattleContext &bc); // end-of-turn tick for every currently channeled orb, left to right
