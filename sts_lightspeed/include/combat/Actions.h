...
namespace sts {
    ...
    struct Actions {
        ...
        // Orb engine (Defect) -- OrbDamageRandomEnemy/OrbDamageAllEnemy/OrbDamageLowestHpEnemy
        // are declared further down, alongside DamageRandomEnemy et al.
        static Action ChannelOrbAction(Orb orb);
        static Action EvokeOrbAction(int slotIdx, int times=1); // slotIdx=-1 means "rightmost" (most recently channeled)
        static Action TriggerOrbPassiveAction(int slotIdx); // slotIdx=-1 means "rightmost"; used by Darkness's immediate trigger
        static Action RecursionAction(); // evoke leftmost orb, then channel it again
        ...
        // Defect orb evoke damage -- same "pure" damage semantics as
        // DamageAllEnemy/DamageRandomEnemy above (this codebase's
        // established convention for non-card-sourced player damage),
        // added alongside Player::evokeOrb's already-implemented targeting
        // logic (Lightning/Dark orbs), which was calling these three but
        // they didn't exist yet.
        static Action OrbDamageAllEnemy(int damage);
        static Action OrbDamageRandomEnemy(int damage);
        static Action OrbDamageLowestHpEnemy(int damage);
        ...
        static Action BulletTimeAction(); // sets costForTurn (not base cost) to 0 for the whole current hand
        ...
        static Action ChooseDiscardCards(int count); // Acrobatics, Prepared, Dagger Throw, Concentrate, Tools of the Trade
        static Action ChooseRetainCards(int count); // Well-Laid Plans
        ...
        static Action WhiteNoiseAction();
        ...
        static Action PoisonLoseHpAction(int idx);
        ...
        // Silent cards needing bespoke helpers (random-target debuff, whole-hand
        // discard, on-draw conditional, random-card generation, potion generation)
        static Action PoisonRandomEnemyAction(int amount, int hits); // Bouncing Flask
        static Action CalculatedGambleAction(); // discard hand, draw that many
        static Action EscapePlanAction(int blockAmount); // draw 1, gain block if it was a Skill
        static Action DistractionAction();
        static Action AlchemizeAction(); // obtain a random potion

        // X-cost Silent cards, mirroring WhirlwindAction/TransmutationAction's
        // energy-consumption pattern (X == energy spent, +2 from Chemical X).
        static Action SkewerAction(int targetIdx, int baseDamage, int energy, bool useEnergy);
        static Action MalaiseAction(int targetIdx, int energy, bool useEnergy, bool upgraded);

        static Action DiscardNonAttackCardsInHandAction(); // Unload
        static Action DiscardRandomCardInHandAction(); // All-Out Attack
    };
