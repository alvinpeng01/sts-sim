...
void CardManager::removeFromHandById(std::int16_t uniqueId) {
...
void CardManager::draw(BattleContext &bc, int amount) {
    ...
    for (int i = 0; i < amount; i++) {
        ...
        if (c.getType() == CardType::SKILL) {
            ...
            if (c.getId() == CardId::DEUS_EX_MACHINA) {
                bc.addToBot(Actions::MakeTempCardInHand(CardId::MIRACLE, true, c.isUpgraded() ? 3 : 2));
        ...
        } else if (c.getType() == CardType::ATTACK) {
            // Resurrected from the commented-out CardInstance::triggerWhenDrawn
            // block below (CardInstance.cpp) -- that method was written but
            // never actually called from anywhere, so neither card's on-draw
            // effect ever fired. This is the real hook point (mirrors how
            // evolve/fireBreathing above react to every card drawn).
            if (c.getId() == CardId::ENDLESS_AGONY) {
                bc.addToBot( Actions::MakeTempCardInHand(c, 1) );
            } else if (c.getId() == CardId::EVISCERATE) {
                c.setCostForTurn(c.cost - bc.player.cardsDiscardedThisTurn);
            }
