...
void Monster::applyStartOfTurnPowers(BattleContext &bc) {
    ...
    if (hasStatus<MS::CHOKED>()) {
        // Also zero the backing amount, not just the bit -- setHasStatus
        // only ever touches statusBits (see its template above), while the
        // additive buff<>() path that applies Choke does `choked += amount`
        // (Monster.h). Without this, a second Choke application in a later
        // turn against the same still-alive enemy would add onto the stale
        // leftover value instead of starting fresh. Not caught until now
        // since nothing ever set this status to a nonzero value before.
        setStatus<MS::CHOKED>(0);
...
void Monster::attacked(BattleContext &bc, int damage) {
    ...
    if (hasStatus<MS::TALK_TO_THE_HAND>()) {
        bc.addToBot(Actions::GainBlock(getStatus<MS::TALK_TO_THE_HAND>()));
    }
