...
Action Actions::ChangeStance(Stance stance) {
    return {[=] (BattleContext &bc) {
        const auto oldStance = bc.player.stance;
        if (oldStance == stance) {
            return;
        }
        // Leaving Calm grants 2 energy; Violet Lotus adds one more.
        if (oldStance == Stance::CALM) {
            bc.player.gainEnergy(2 + (bc.player.hasRelic<RelicId::VIOLET_LOTUS>() ? 1 : 0));
        }
        bc.player.stance = stance;
        if (stance == Stance::DIVINITY) bc.player.gainEnergy(3);
        if (bc.player.hasStatus<PS::MENTAL_FORTRESS>()) {
            bc.player.gainBlock(bc, bc.player.getStatus<PS::MENTAL_FORTRESS>());
        }
        if (stance == Stance::WRATH && bc.player.hasStatus<PS::RUSHDOWN>()) {
            bc.drawCards(bc.player.getStatus<PS::RUSHDOWN>());
        }
        for (int i = static_cast<int>(bc.cards.discardPile.size()) - 1; i >= 0; --i) {
            if (bc.cards.discardPile[i].id == CardId::FLURRY_OF_BLOWS) {
                const auto flurry = bc.cards.discardPile[i];
                bc.cards.removeFromDiscard(i);
                bc.moveToHandHelper(flurry);
            }
        }
    }};
}
...
Action Actions::Scry(int amount) {
    return {[=] (BattleContext &bc) {
        // Golden Eye modifies the Scry effect itself, including Melange.
        int actualAmount = amount;
        if (bc.player.hasRelic<RelicId::GOLDEN_EYE>()) {
            actualAmount += 2;
        }
        bc.scryCount = std::min(actualAmount, static_cast<int>(bc.cards.drawPile.size()));
        if (bc.player.hasStatus<PS::NIRVANA>()) {
            bc.player.gainBlock(bc, bc.player.getStatus<PS::NIRVANA>());
        }
        // Every Weave in the discard pile returns when a Scry is performed,
        // provided the hand has room.
        for (int i = static_cast<int>(bc.cards.discardPile.size()) - 1;
             i >= 0 && bc.cards.cardsInHand < CardManager::MAX_HAND_SIZE; --i) {
            if (bc.cards.discardPile[i].id == CardId::WEAVE) {
                const auto weave = bc.cards.discardPile[i];
                bc.cards.removeFromDiscard(i);
                bc.moveToHandHelper(weave);
            }
        }
        if (bc.scryCount > 0) {
            bc.setState(InputState::SCRY);
        }
    }};
}
...
Action Actions::TriggerEndOfTurnOrbsAction() {
    return {[] (BattleContext &bc) {
        bc.player.triggerOrbPassives(bc);
        // Loop retriggers the leftmost orb after the normal end-of-turn
        // orb passives.  The upgraded card supplies two retriggers.
        const int loopCount = bc.player.getStatus<PS::LOOP>();
...
Action Actions::ChannelOrbAction(Orb orb) {
    return {[=] (BattleContext &bc) {
        bc.player.channelOrb(bc, orb);
    }};
}

Action Actions::EvokeOrbAction(int slotIdx, int times) {
    return {[=] (BattleContext &bc) {
        const int idx = slotIdx < 0 ? bc.player.orbCount - 1 : slotIdx;
        bc.player.evokeOrb(bc, idx, times);
    }};
}

Action Actions::TriggerOrbPassiveAction(int slotIdx) {
    return {[=] (BattleContext &bc) {
        const int idx = slotIdx < 0 ? bc.player.orbCount - 1 : slotIdx;
        bc.player.triggerOrbPassive(bc, idx);
    }};
}

Action Actions::RecursionAction() {
    return {[] (BattleContext &bc) {
        if (bc.player.orbCount <= 0) {
            return;
        }
        const Orb orb = bc.player.orbs[0];
...
Action Actions::OrbDamageAllEnemy(int baseDamage) {
    return {[=] (BattleContext &bc) {
        for (int idx = 0; idx < bc.monsters.monsterCount; idx++) {
            if (!bc.monsters.arr[idx].isDeadOrEscaped()) {
                bc.monsters.arr[idx].damage(bc, bc.calculateOrbDamage(idx, baseDamage));
            }
        }
        bc.checkCombat();
    }};
...
Action Actions::OrbDamageRandomEnemy(int baseDamage) {
    return {[=] (BattleContext &bc) {
        const int idx = bc.monsters.getRandomMonsterIdx(bc.cardRandomRng, true);
        if (idx == -1) {
            return;
...
Action Actions::OrbDamageLowestHpEnemy(int baseDamage) {
    return {[=] (BattleContext &bc) {
        const int idx = bc.monsters.getLowestHpMonsterIdx(true);
        if (idx == -1) {
            return;
        }
        bc.monsters.arr[idx].damage(bc, bc.calculateOrbDamage(idx, baseDamage));
...
Action Actions::BulletTimeAction() {
    // Unlike RandomizeHandCost above, this only touches costForTurn, not
    // the persistent `cost` field -- Bullet Time's own text is "this turn"
    // only, the cards should be back to their normal cost next turn.
    return {[=] (BattleContext &bc) {
        for (int i = 0; i < bc.cards.cardsInHand; ++i) {
            bc.cards.hand[i].setCostForTurn(0);
        }
    }};
}
...
Action Actions::PutRandomCardsInDrawPile(CardType type, int count) {
    return {[=] (BattleContext &bc) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            // Chrysalis and Metamorphosis make their generated cards free
            // for the entire combat, including the turn they are drawn.
...
Action Actions::DistractionAction() {
    return {[=] (BattleContext &bc) {
        const auto cid = getTrulyRandomCardInCombat(bc.cardRandomRng, bc.player.cc, CardType::SKILL);
        CardInstance c(cid);
        c.setCostForTurn(0);
...
Action Actions::AlchemizeAction() {
    return {[=] (BattleContext &bc) {
        Potion p = returnRandomPotion(bc.potionRng, bc.player.cc, true);
        bc.obtainPotion(p);
    }};
}

// Bouncing Flask: apply `amount` Poison to a random enemy, `hits` separate times
// (each hit independently rerolls its target, matching AttackAllEnemy/DamageRandomEnemy's
// per-call getRandomMonsterIdx pattern elsewhere in this file).
Action Actions::PoisonRandomEnemyAction(int amount, int hits) {
    return {[=] (BattleContext &bc) {
        for (int i = 0; i < hits; ++i) {
            ...
            bc.debuffEnemy<MS::POISON>(idx, amount, false);
        }
    }};
}

// Calculated Gamble: by the time this queued action runs, the card itself has
// already been removed from hand (BattleContext::useCard removes it synchronously
// right after the useSkillCard() switch returns, before the action queue drains --
// see the EXPERTISE case comment for the same timing fact), so cardsInHand here
// reflects only the rest of the hand. Mirrors BattleContext::discardAtEndOfTurnHelper's
// discard-whole-hand pattern, including the cardsDiscardedThisTurn bookkeeping.
Action Actions::CalculatedGambleAction() {
    return {[=] (BattleContext &bc) {
        ...
        bc.addToBot( Actions::DrawCards(count) );
    }};
}

// Escape Plan: draw 1 card, gain block only if the card drawn was a Skill.
// CardManager::draw appends to the end of hand (moveToHand), so the newly
// drawn card is always at hand[cardsInHand-1] right after the draw call.
// Can't just call bc.cards.draw(bc, 1) directly -- CardManager::popFromDrawPile
// UB's on an empty draw pile; BattleContext::drawCards() is the safe entry point
// but it defers to a queued reshuffle instead of resolving synchronously, so the
// "was the drawn card a Skill" check couldn't see the result. Mirrors the
// reshuffle-then-draw guard from BattleContext::drawCards / Actions::EmptyDeckShuffle
// inline so the draw resolves synchronously here.
Action Actions::EscapePlanAction(int blockAmount) {
    return {[=] (BattleContext &bc) {
        if (bc.player.hasStatus<PS::NO_DRAW>() || bc.cards.cardsInHand >= 10 ||
            (bc.cards.drawPile.empty() && bc.cards.discardPile.empty())) {
            return;
        }
        ...
        const int handBefore = bc.cards.cardsInHand;
        bc.cards.draw(bc, 1);
        if (bc.cards.cardsInHand > handBefore) {
            const CardInstance &drawn = bc.cards.hand[bc.cards.cardsInHand - 1];
            if (drawn.getType() == CardType::SKILL) {
                bc.addToBot( Actions::GainBlock(blockAmount) );
            }
        }
    }};
}
...
Action Actions::WhiteNoiseAction() {
    return {[] (BattleContext &bc) {
        const auto cid = getTrulyRandomCardInCombat(bc.cardRandomRng, bc.player.cc, CardType::POWER);
        CardInstance c(cid);
        c.setCostForTurn(0);
        bc.addToTop(Actions::MakeTempCardInHand(c));
    }};
...
Action Actions::BetterDiscardPileToHandAction(int amount, CardSelectTask task) {
    return {[=] (BattleContext &bc) {
        ...
        if (task == CardSelectTask::MEDITATE) {
            bc.cardSelectInfo.cardSelectTask = task;
            bc.cardSelectInfo.pickCount = std::min(amount, static_cast<int>(bc.cards.discardPile.size()));
...
Action Actions::EssenceOfDarkness(int darkOrbsPerSlot) {
    return {[=] (BattleContext &bc) {
        const int count = bc.player.orbSlots * darkOrbsPerSlot;
        for (int i = 0; i < count; ++i) {
...
Action Actions::PoisonLoseHpAction(int idx) {
    // Was a stub (`return sts::Action();`, a default-constructed empty
    // Action) -- Poison's status bookkeeping (stacking, application via
    // cards/potions/The Specimen) was fully wired up, but the actual
    // "lose HP equal to Poison stacks, then Poison decreases by 1" tick
    // that's the entire reason the status exists was never implemented.
    // Invoking the empty Action later crashed with bad_function_call
    // (an empty std::function called), reproducible with just two Poison
    // applications before ending a turn -- found while implementing
    // Silent's poison cards, not a regression from that work.
    return {[=] (BattleContext &bc) {
        Monster &m = bc.monsters.arr[idx];
        if (m.isDeadOrEscaped()) {
            return;
        }
        const int amount = m.getStatus<MS::POISON>();
        bc.addToTop( Actions::DamageEnemy(idx, amount) );
        m.decrementStatus<MS::POISON>(1);
    }};
}
...
Action Actions::ChooseDiscardCards(int count) {
    return {[=] (BattleContext &bc) {
        if (bc.cards.cardsInHand == 0) {
            return;
        }
        ...
        // Real STS forces discarding exactly this many, or your whole hand
        // if you have fewer -- not "up to N, your choice of fewer" like
        // Exhaust Many/Gamble above.
        bc.cardSelectInfo.pickCount = std::min(count, static_cast<int>(bc.cards.cardsInHand));
    }};
}

Action Actions::ChooseRetainCards(int count) {
    return {[=] (BattleContext &bc) {
        if (bc.cards.cardsInHand == 0) {
            return;
        }
        // "Retain up to N" -- an actual "up to", unlike ChooseDiscardCards
        // above -- same shape as EXHAUST_MANY's "up to N" semantics, so
        // isValidMultiCardSelectAction's DISCARD case (exact count) doesn't
        // apply here; RETAIN is validated like EXHAUST_MANY instead.
        bc.inputState = InputState::CARD_SELECT;
        bc.cardSelectInfo.cardSelectTask = CardSelectTask::RETAIN;
...
Action Actions::SkewerAction(int targetIdx, int baseDamage, int energy, bool useEnergy) {
    return {[=] (BattleContext &bc) {
        if (useEnergy) {
            bc.player.useEnergy(bc.player.energy);
        }
        const auto effectAmount = energy + (bc.player.hasRelic<R::CHEMICAL_X>() ? 2 : 0);
        for (int i = 0; i < effectAmount; ++i) {
            bc.addToBot( Actions::AttackEnemy(targetIdx, baseDamage) );
        }
    }};
}

Action Actions::MalaiseAction(int targetIdx, int energy, bool useEnergy, bool upgraded) {
    return {[=] (BattleContext &bc) {
        if (useEnergy) {
            bc.player.useEnergy(bc.player.energy);
        }
        const auto effectAmount = energy + (bc.player.hasRelic<R::CHEMICAL_X>() ? 2 : 0) + (upgraded ? 1 : 0);
        if (effectAmount > 0) {
            bc.addToBot( Actions::DebuffEnemy<MS::STRENGTH>(targetIdx, -effectAmount, false) );
            bc.addToBot( Actions::DebuffEnemy<MS::WEAK>(targetIdx, effectAmount, false) );
        }
    }};
}

// Unload: discard all non-Attack cards in hand (no player choice -- deterministic filter).
Action Actions::DiscardNonAttackCardsInHandAction() {
    return {[=] (BattleContext &bc) {
        for (int i = bc.cards.cardsInHand - 1; i >= 0; --i) {
...
// All-Out Attack: discard 1 card at random (no-op if hand is empty).
Action Actions::DiscardRandomCardInHandAction() {
    return {[=] (BattleContext &bc) {
        if (bc.cards.cardsInHand <= 0) {
            return;
        }
