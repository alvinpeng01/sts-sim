    ...
    switch (moveHistory[0]) {
        ...
        case MMID::LAGAVULIN_SLEEP:
            if (bc.turn == 2 || !hasStatus<MS::ASLEEP>()) {
                // Scheduled wake. The damage-triggered wake (Monster.cpp:401 and
                // :465) clears ASLEEP *and* removes the 8 Metallicize; this path
                // used to do neither, so a Lagavulin left alone until turn 3 woke
                // up still armoured and regained 8 block every turn for the rest
                // of the fight -- roughly 64 damage against its 111-115 HP. The
                // game's ARMOR_AMT = 8 has no ascension tier, so the bug was
                // identical at A0 and A20. Guarded on ASLEEP because this branch
                // is also taken when already awake, where decrementing again
                // would remove Metallicize the monster never had.
                // See docs/11-engine-validation.md.
                if (hasStatus<MS::ASLEEP>()) {
      ...
      assert(curHp > 0);
        ...
        case MMID::THE_CHAMP_GLOAT: {
            // The game's TheChamp constructor: STR_AMT = 2, A_4_STR_AMT = 3,
            // A_19_STR_AMT = 4 (verified against desktop-1.0.jar, see
            // docs/11-engine-validation.md). Ours read {3, 4, 5} -- one tier too
            // high at every ascension, so 3 vs 2 at A0 and 5 vs 4 at A20, on the
            // Act 2 boss where runs die.
        ...
        case MMID::DARKLING_CHOMP:
            // Two hits, not one: the game's Darkling has CHOMP_AMT = 2
            // (verified against desktop-1.0.jar, and silverbot-reference cites
            // the same constant). Ours dealt half the real damage -- 8 against
            // 16 at A0, 9 against 18 at A20. Act 3, so it bites hardest at A0
            // where the agent reaches floor ~39. See docs/11-engine-validation.md.
        ...
        case MMID::WRITHING_MASS_FLAIL: // 2
            // Block equals the flail's own damage. The game builds this
            // GainBlockAction from the SAME DamageInfo as the attack, so there
            // is no separate block constant to read -- and no 18 anywhere in
            // WrithingMass.class. Its damage list is {32/38, 7/9, 15/16, 10/12}
            // and the flail is index 2, giving 15 base and 16 at A2+. Ours had
            // an invented 16/18. See docs/11-engine-validation.md.
            attackPlayerHelper(bc, asc2 ? 16 : 15);
