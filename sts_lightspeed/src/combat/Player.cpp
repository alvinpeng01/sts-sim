...
int Player::getStatusRuntime(PlayerStatus s) const {
    switch (s) {
        ...
        case PS::STRENGTH:
            return strength;
        // These five are bool-only in buff<>/debuff<>'s template (see
        // Player.h) -- setHasStatus<s>(true) is called directly and
        // statusMap is deliberately never touched, since there's no
        // meaningful stack count for them, only presence. getStatusRuntime
        // used to fall through to statusMap.at(s) for every non-special-
        // cased status, which throws std::out_of_range for these five the
        // first time they're ever active -- confirmed via a real repro
        // (playing Barricade, then querying it, threw map::at). Presence
        // is the only thing that was ever meaningful here, so 1/0 is the
        // correct value, not a worked-around crash.
        case PS::BARRICADE:
        case PS::CORRUPTION:
        case PS::CONFUSED:
        case PS::PEN_NIB:
        case PS::SURROUNDED:
            return hasStatusRuntime(s) ? 1 : 0;
        default:
...
void Player::gainBlock(BattleContext &bc, int amount) {
    ...
    if (hasStatus<PS::WAVE_OF_THE_HAND>()) {
        const int target = bc.monsters.getRandomMonsterIdx(bc.cardRandomRng);
        if (target >= 0 && target < bc.monsters.monsterCount && bc.monsters.arr[target].isTargetable()) {
            bc.addToBot(Actions::DebuffEnemy<MS::WEAK>(target, getStatus<PS::WAVE_OF_THE_HAND>(), false));
        }
    }
}
...
void Player::increaseOrbSlots(int amount) {
    orbSlots = static_cast<std::int8_t>(std::min<int>(orbSlots + amount, MAX_ORB_SLOTS));
}

bool Player::hasEmptyOrb() const {
    return orbCount < orbSlots;
}

void Player::channelOrb(BattleContext &bc, Orb orb) {
    if (orbSlots <= 0) {
        return; // no orb slots at all (not a Defect run, or not yet granted any)
    }
    if (orbCount >= orbSlots) {
        evokeOrb(bc, 0); // no empty slot -- the leftmost/oldest orb is evoked to make room
    }
    orbs[orbCount] = orb;
    if (orb == Orb::LIGHTNING) {
        ++lightningChanneledThisCombat;
    } else if (orb == Orb::FROST) {
        ++frostChanneledThisCombat;
    }
    if (orb == Orb::DARK) {
        orbDarkAmount[orbCount] = static_cast<std::int16_t>(6 + getStatus<PS::FOCUS>());
    }
    ++orbCount;
}

void Player::evokeOrb(BattleContext &bc, int slotIdx, int times) {
    if (slotIdx < 0 || slotIdx >= orbCount) {
        return;
    }
    const Orb orb = orbs[slotIdx];

    // Dark's evoke amount is a fixed read of its current stored value, deliberately not
    // incremented here (that only happens on the passive tick) -- Dualcast evoking the same
    // orb "twice" (times=2) reads the identical amount both times, matching the real game.
    for (int t = 0; t < times; ++t) {
        switch (orb) {
            case Orb::LIGHTNING: {
                const int dmg = 8 + getStatus<PS::FOCUS>();
                if (hasStatus<PS::ELECTRO>()) {
                    bc.addToBot( Actions::OrbDamageAllEnemy(dmg) );
                } else {
                    bc.addToBot( Actions::OrbDamageRandomEnemy(dmg) );
                }
                break;
            }
            case Orb::FROST:
                bc.addToBot( Actions::GainBlock(5 + getStatus<PS::FOCUS>()) );
                break;
            case Orb::DARK:
                bc.addToBot( Actions::OrbDamageLowestHpEnemy(orbDarkAmount[slotIdx]) );
                break;
            case Orb::PLASMA:
    ...
    for (int i = slotIdx; i < orbCount - 1; ++i) {
        orbs[i] = orbs[i + 1];
        orbDarkAmount[i] = orbDarkAmount[i + 1];
    }
    --orbCount;
    orbs[orbCount] = Orb::EMPTY;
}

void Player::triggerOrbPassive(BattleContext &bc, int slotIdx) {
    if (slotIdx < 0 || slotIdx >= orbCount) {
        return;
    }
    switch (orbs[slotIdx]) {
        case Orb::LIGHTNING: {
            const int dmg = 3 + getStatus<PS::FOCUS>();
            if (hasStatus<PS::ELECTRO>()) {
                bc.addToBot( Actions::OrbDamageAllEnemy(dmg) );
            } else {
                bc.addToBot( Actions::OrbDamageRandomEnemy(dmg) );
        ...
        case Orb::FROST:
            bc.addToBot( Actions::GainBlock(2 + getStatus<PS::FOCUS>()) );
            break;
        case Orb::DARK:
            bc.addToBot( Actions::OrbDamageLowestHpEnemy(orbDarkAmount[slotIdx]) );
            orbDarkAmount[slotIdx] = static_cast<std::int16_t>(orbDarkAmount[slotIdx] + 6 + getStatus<PS::FOCUS>());
            break;
        case Orb::PLASMA:
...
void Player::triggerOrbPassives(BattleContext &bc) {
    for (int i = 0; i < orbCount; ++i) {
        triggerOrbPassive(bc, i);
    }

    // Gold-Plated Cables: "Your rightmost orb triggers its passive twice."
    // Was in the Defect uncommon pool and read by no code, so it did nothing.
    // Orbs are stored left-to-right, so the rightmost is the last filled slot.
    // Defect-only relic: no Ironclad number moves.
    if (orbCount > 0 && hasRelic<R::GOLD_PLATED_CABLES>()) {
        triggerOrbPassive(bc, orbCount - 1);
    }
}
...
void Player::damage(BattleContext &bc, const int calculatedDamage, const bool selfDamage) {
    ...
    if (stance == Stance::WRATH) damage *= 2;
...
void Player::attacked(BattleContext &bc, int enemyIdx, int calculatedDamage) {
    ...
    if (damage > 0 && hasStatus<PS::STATIC_DISCHARGE>()) {
        for (int i = 0; i < getStatus<PS::STATIC_DISCHARGE>(); ++i) {
            channelOrb(bc, Orb::LIGHTNING);
        }
    }
...
void Player::applyEndOfTurnPowers(BattleContext &bc) {
    ...
    for (auto pair : statusMap) {
        ...
        switch (pair.first) {
            ...
            case PS::WELL_LAID_PLANS:
                // A persistent power (not consumed/removed here, unlike the
                // one-shot DRAW_CARD_NEXT_TURN-style statuses elsewhere in
                // this switch) -- retriggers every end of turn for the rest
                // of combat, same as Demon Form/Noxious Fumes's own
                // every-turn re-application.
                bc.addToBot( Actions::ChooseRetainCards(pair.second) );
                break;
        ...
        switch (pair.first) {
            ...
            case PS::CREATIVE_AI:
                for (int i = 0; i < pair.second; ++i) {
                    const auto id = getTrulyRandomCardInCombat(bc.cardRandomRng, cc, CardType::POWER);
                    bc.addToBot(Actions::MakeTempCardInHand(CardInstance(id)));
                }
                break;
            ...
            case PS::FORESIGHT:
                // At this point the turn's normal draw has already been queued;
                // use the same shuffle path as drawCards when necessary.
                if (bc.cards.drawPile.empty() && !bc.cards.discardPile.empty()) {
                    bc.onShuffle();
                    bc.addToBot(Actions::EmptyDeckShuffle());
                }
                bc.addToBot(Actions::Scry(pair.second));
                break;
            ...
            case PS::HELLO_WORLD:
                for (int i = 0; i < pair.second; ++i) {
                    const auto id = getRandomClassCardOfRarity(
                        bc.cardRandomRng, CharacterClass::DEFECT, CardRarity::COMMON);
                    bc.addToBot(Actions::MakeTempCardInHand(CardInstance(id)));
                }
                break;
            ...
            case PS::MAGNETISM:
                for (int i = 0; i < pair.second; ++i) {
                    const auto id = getTrulyRandomColorlessCardInCombat(bc.cardRandomRng);
                    bc.addToBot(Actions::MakeTempCardInHand(CardInstance(id)));
                }
                break;
...
void Player::applyStartOfTurnPostDrawPowers(BattleContext &bc) {
    ...
    for (auto pair :statusMap) {
        ...
        switch (pair.first) {
            ...
            case PS::TOOLS_OF_THE_TRADE:
                bc.addToBot( Actions::DrawCards(pair.second) );
                bc.addToBot( Actions::ChooseDiscardCards(pair.second) );
                break;
    ...
    // Nightmare: doesn't fit the statusMap loop above since it needs to
    // carry an arbitrary CardId, not just an int amount -- see
    // nightmareCardId's comment in Player.h. Mirrors the real game's own
    // "todo case NightMarePower" note that used to sit where the Nightmare
    // card's case is in BattleContext::useSkillCard(): add the copies, then
    // clear the pending state (one-shot, like DRAW_CARD_NEXT_TURN above).
    if (nightmareCardId != CardId::INVALID) {
        bc.addToBot( Actions::MakeTempCardInHand(nightmareCardId, false, nightmareCopies) );
        nightmareCardId = CardId::INVALID;
        nightmareCopies = 0;
    }
}
...
void Player::rechargeEnergy(BattleContext &bc) {
    ...
    if (hasStatus<PS::FASTING>()) {
        energy = std::max(0, energy - getStatus<PS::FASTING>());
    }

    // ****** Player powers onEnergyRecharge ******
    if (hasStatus<PS::COLLECT>()) {
        decrementStatus<PS::COLLECT>();
        // Collect explicitly creates Miracle+, independent of Master Reality.
        bc.addToBot(Actions::MakeTempCardInHand(CardId::MIRACLE, true) );
    }
