...
//    std::cout << *this << std::endl;
    ...
    switch (r) {
        ...
        // Dolly's Mirror sat in the shop pool and was read by no code, so it
        // did nothing. The game's DollysMirror.onEquip calls
        // makeStatEquivalentCopy: the player picks a card and gains a copy
        // carrying its upgrade state and accumulated stats. The DUPLICATE
        // select screen already implements exactly that -- the Duplicator
        // event uses it, and its resolution is `deck.obtain(*this, c.card, 1)`,
        // which copies the Card itself rather than a fresh one of that id.
