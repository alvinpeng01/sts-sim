...
void search::SimpleAgent::playoutBattle(BattleContext &bc) {
    ...
    while (bc.outcome == Outcome::UNDECIDED) {
        ...
        } else if (bc.inputState == InputState::SCRY) {
            // The lightweight rollout agent has no deck-quality heuristic;
            // preserve the deck rather than stalling the combat.
            takeAction(bc, Action(ActionType::SCRY, 0));
