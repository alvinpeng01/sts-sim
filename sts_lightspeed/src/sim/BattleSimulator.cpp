...
void BattleSimulator::printActions(std::ostream &os) const {
    ...
    switch (bc->inputState) {
        ...
        case InputState::SCRY:
            os << "Scry: enter zero or more indexes to discard (0 is drawn next)\n";
            for (int i = 0; i < bc->scryCount; ++i) {
                os << i << ": " << bc->cards.drawPile[bc->cards.drawPile.size() - 1 - i] << "\n";
            }
            break;
...
void BattleSimulator::printCardSelectActions(std::ostream &os) const {
    switch (bc->cardSelectInfo.cardSelectTask) {
        ...
        case CardSelectTask::FOREIGN_INFLUENCE:
            os << "Foreign Influence: Choose an Attack to add to your hand.\n";
            printCardOptionsHelper(os, bc->cardSelectInfo.cards.begin(), bc->cardSelectInfo.cards.end());
            break;

        case CardSelectTask::WISH:
            os << "Wish: Choose a reward.\n";
            printCardOptionsHelper(os, bc->cardSelectInfo.cards.begin(), bc->cardSelectInfo.cards.end());
            break;
        ...
        case CardSelectTask::OMNISCIENCE:
            os << "Omniscience: Choose a card from your draw pile to play twice.\n";
        ...
        case CardSelectTask::SEEK:
            os << "Seek: Choose card(s) from your draw pile to put in your hand.\n";
            printCardOptionsHelper(os, bc->cards.drawPile.begin(), bc->cards.drawPile.end());
            break;
        ...
        case CardSelectTask::RECYCLE:
            os << "Recycle: Choose a card to exhaust. Gain energy equal to its cost.\n";
            printCardOptionsHelper(os, bc->cards.hand.begin(), bc->cards.hand.begin() + bc->cards.cardsInHand);
            break;
...
void BattleSimulator::takeCardSelectAction(const std::string &action) {
    switch (bc->cardSelectInfo.cardSelectTask) {
        ...
        case CardSelectTask::FOREIGN_INFLUENCE:
            bc->chooseForeignInfluenceCard(bc->cardSelectInfo.cards[std::stoi(action)]);
            break;

        case CardSelectTask::WISH:
            bc->chooseWishCard(bc->cardSelectInfo.cards[std::stoi(action)]);
            break;
        ...
        case CardSelectTask::MEDITATE:
            bc->chooseMeditateCards(getIdxListFromString(action));
            break;

        case CardSelectTask::OMNISCIENCE:
            bc->chooseOmniscienceCard(std::stoi(action));
            break;
        ...
        case CardSelectTask::RECYCLE:
            bc->chooseRecycleCard(std::stoi(action));
            break;
        ...
        case CardSelectTask::SEEK:
            bc->chooseSeekCards(getIdxListFromString(action));
            break;
